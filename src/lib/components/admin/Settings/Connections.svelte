<script lang="ts">
	...
	let PIPELINES_API_BASE_URLS = [''];

	let pipelineUrls = {};

	let ENABLE_OPENAI_API = null;
	let ENABLE_OLLAMA_API = null;
	let ENABLE_PIPELINES_API = null;
	...
	const verifyPipelinesHandler = async (idx) => {
		PIPELINES_API_BASE_URLS = PIPELINES_API_BASE_URLS.filter((url) => url !== '').map((url) =>
			url.replace(/\/$/, '')
		);

		PIPELINES_API_BASE_URLS = await updatePipelineUrls(localStorage.token, PIPELINES_API_BASE_URLS);

		const res = await getPipelines(localStorage.token, idx).catch((error) => {
			toast.error(error);
			return null;
		});
	...
	const updatePipelinesUrlsHandler = async () => {
		PIPELINES_API_BASE_URLS = PIPELINES_API_BASE_URLS.filter((url) => url !== '').map((url) =>
			url.replace(/\/$/, '')
		);

		console.log(PIPELINES_API_BASE_URLS);

		if (PIPELINES_API_BASE_URLS.length === 0) {
			ENABLE_PIPELINES_API = false;
			await updatePipelineUrls(localStorage.token, ENABLE_PIPELINES_API);

			toast.info($i18n.t('Pipelines API disabled'));
		} else {
			PIPELINES_API_BASE_URLS = await updatePipelineUrls(localStorage.token, PIPELINES_API_BASE_URLS);

			const pipelinesList = await getPipelinesList(localStorage.token).catch((error) => {
				toast.error(error);
				return null;
			});
	...
	onMount(async () => {
		if ($user.role === 'admin') {
			await Promise.all([
				...
				(async () => {
					PIPELINES_API_BASE_URLS = await getPipelinesList(localStorage.token);
				})()
			]);
			...
			ENABLE_PIPELINES_API = true; // Assuming pipelines API is enabled by default
...
<form
	...
	on:submit|preventDefault={() => {
		...
		updatePipelinesUrlsHandler();

		dispatch('save');
	}}
>
	<div class="space-y-3 overflow-y-scroll scrollbar-hidden h-full">
		{#if ENABLE_OPENAI_API !== null && ENABLE_OLLAMA_API !== null && ENABLE_PIPELINES_API !== null}
			<div class=" space-y-3">
			...
			<div class="pr-1.5 space-y-2">
				<div class="flex justify-between items-center text-sm">
					<div class="  font-medium">{$i18n.t('Pipelines API')}</div>

					<div class="mt-1">
						<Switch
							bind:state={ENABLE_PIPELINES_API}
							on:change={async () => {
								updatePipelineUrls(localStorage.token, ENABLE_PIPELINES_API);

								if (PIPELINES_API_BASE_URLS.length === 0) {
									PIPELINES_API_BASE_URLS = [''];
								}
							}}
						/>
					</div>
				</div>
				{#if ENABLE_PIPELINES_API}
					<div class="flex w-full gap-1.5">
						<div class="flex-1 flex flex-col gap-2">
							{#each PIPELINES_API_BASE_URLS as url, idx}
								<div class="flex gap-1.5">
									<input
										class="w-full rounded-lg py-2 px-4 text-sm bg-gray-50 dark:text-gray-300 dark:bg-gray-850 outline-none"
										placeholder={$i18n.t('Enter URL (e.g. http://localhost:11434)')}
									...
									<div class="self-center flex items-center">
										{#if idx === 0}
											<button
												class="px-1"
												on:click={() => {
													PIPELINES_API_BASE_URLS = [...PIPELINES_API_BASE_URLS, ''];
												}}
												type="button"
											>
												<svg
										...
										{:else}
											<button
												class="px-1"
												on:click={() => {
													PIPELINES_API_BASE_URLS = PIPELINES_API_BASE_URLS.filter(
														(url, urlIdx) => idx !== urlIdx
													);
												}}
												type="button"
					...
					<div class="mt-2 text-xs text-gray-400 dark:text-gray-500">
						{$i18n.t('Trouble accessing Pipelines?')}
						<a
							class=" text-gray-300 font-medium underline"
							href="https://github.com/open-webui/open-webui#troubleshooting"
							target="_blank"
